...
$ ->
  $(".purchase-form").on "ajax:success", (event, data, status, xhr) ->
    ...
    if data.status
      $this.parents(".item-box").removeClass("book-not-purchased").addClass "book-purchased"
  ...
  $(".cancel-button").on "ajax:success", (event, data, status, xhr) ->
    ...
    return if ajaxResponseStatusFilter(data)
    if data.status
      $this.parents(".item-box").addClass("book-not-purchased").removeClass "book-purchased"
